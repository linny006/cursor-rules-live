...
공개 GitHub 저장소에서 `.cursorrules` 및 `cursorrules.json` 파일을 15분마다 자동으로 탐색하고 인덱싱합니다. 실제 팀들이 지금 사용 중인 규칙을 언어, Star 수, 마지막 업데이트 시각 등의 메타데이터와 함께 보여줍니다. 검색 가능한 JSON 피드와 정적 사이트를 생성해 최신 AI 코딩 규칙을 편하게 탐색할 수 있습니다.

이 목록은 GitHub Actions cron에 의해 **15분마다 자동 업데이트**됩니다. 각 커밋은 업스트림 데이터 소스의 실제 변경 사항(신규 추가, 만료 항목 제거)을 반영하므로 표시된 내용이 항상 최신임을 신뢰할 수 있습니다.
